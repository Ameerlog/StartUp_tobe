...
const stepIcons = { 1: Building2, 2: User, 3: FileCheck };
...
const GlassCard = ({
  ...
  <div className={`relative group ${className} ${hover}`}>
    <div
...
const CoVentureBrandListingForm = ({ onSuccess }) => {
  ...
  const {
    ...
    error: submitError,
  } = useMutation(coventureAPI.create);
  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case "description":
        ...
        if (value.length > 73) return "Must not exceed 73 characters";
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      brandDetails: {
        ...
        industry: formData.industryCategory?.toUpperCase(),
        dealValue: parseInt(formData.coVenturePrice?.replace(/,/g, "") || "0"),
    ...
    try {
      ...
      // ✅ Show toast
      toast.success("Co-branding partnership created successfully!");
    } catch (error) {
              ...
              <AnimatePresence mode="wait">
                {currentStep === 1 && (
                  ...
                  >
                    ...
                    {/* Brand Description Field - Optional */}
                    <motion.div
                      initial={{ opacity: 0, y: 15 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <span
                          className={`text-xs ${
                            formData.brandDescription?.length > 450
                              ? "text-orange-400"
                              : "text-neutral-500"
                          }`}
                        >
